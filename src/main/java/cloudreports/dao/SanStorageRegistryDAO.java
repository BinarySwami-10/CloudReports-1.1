...
public class SanStorageRegistryDAO {
	...
	/**
	 * Inserts a new SAN registry into the database. The name of the SAN must be
	 * unique.
	 *
	 * @param sr the SAN registry to be inserted.
	 * @return <code>true</code> if the SAN registry has been successfully inserted;
	 ...
	 * @return the SAN, if it exists; <code>null</code> otherwise.
	 * @see SanStorageRegistry
	 * @since 1.0
	 */
	public SanStorageRegistry getSanStorageRegistry(long sanStorageId) {
		...
		try {
			sanStorage = (SanStorageRegistry) session.createCriteria(SanStorageRegistry.class)
					.add(Restrictions.eq("id", sanStorageId)).uniqueResult();
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, null, ex);
		} finally {
			HibernateUtil.closeSession(session);
