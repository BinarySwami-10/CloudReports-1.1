...
/**
 * MigrationDAO provides basic CRUD operations related to the {@link Migration}
 * class.
 * 
 * @see Migration
 * @author Thiago T. Sá
...
public class MigrationDAO {
	 ...
	 *
	 * @return a list containing all migration occurrences; <code>null</code> if no
	...
	public List<Migration> getMigrationList(String datacenterName) {
		...
		try {
			migrationList = (List<Migration>) session.createCriteria(Migration.class)
					.add(Restrictions.eq("datacenterName", datacenterName)).list();
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, null, ex);
		} finally {
			HibernateUtil.closeSession(session);
