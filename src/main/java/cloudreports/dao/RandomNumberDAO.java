 ...
 * obtained from the QRBG service. It consumes the random numbers pool and adds
 * more entries on demand.
 * 
...
public class RandomNumberDAO {
	...
	/**
	 * The amount of requests to be executed each time the random numbers pool needs
	 * more entries from the QRBG service.
	 */
	private final int NUMBER_OF_REQUESTS = 10;

	/**
	 * Inserts more entries in the random numbers pool. Numbers are obtained from
	 * the QRBG service. The total amount of integers inserted can be calculated as
	 * ({@link #NUMBER_OF_BYTES}/4 * {@link #NUMBER_OF_REQUESTS}.
	 *
	...
	/**
	 * Gets the given amount of random numbers from the random numbers pool. The
	 * effectiveness of this method depends on a proper configuration of a valid
	 * account on the QRBG service and an active Internet connection. Also, the QRBG
	 * service uses the port 1227. Make sure your firewall is configured properly.
	 *
	...
	/**
	 * Gets a list of {@link RandomNumber} registries from the random numbers pool.
	 *
	 * @param amount the amount of random numbers to get from the pool.
	 * @return a list of random numbers from the pool. The resulting list has, at
	 *         most, the number of elements given by <code>amount</code>.
	 * @see RandomNumber
	 * @since 1.0
	 */
