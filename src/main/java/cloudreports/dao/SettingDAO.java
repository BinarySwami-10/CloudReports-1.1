...
/**
 * SettingDAO provides basic CRUD operations related to the {@link Setting}
 * class.
 * 
 * @see Setting
 * @author Thiago T. Sá
...
public class SettingDAO {
	 ...
	 * @return the setting with the given name, if it exists; <code>null</code>
	 *         otherwise.
	 * @see Setting
	 * @since 1.0
	 */
	public Setting getSetting(String name) {
		...
		Session session = HibernateUtil.getSession();
		try {
			setting = (Setting) session.createCriteria(Setting.class).add(Restrictions.eq("name", name)).uniqueResult();
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, null, ex);
		} finally {
			HibernateUtil.closeSession(session);
