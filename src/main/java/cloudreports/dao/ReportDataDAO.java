...
public class ReportDataDAO {
	 ...
	 * @param time           the instant of time which the data being inserted
	 *                       refers to.
	 * @param amount         the amount of resources being used by the host at
	 *                       <code>time</code>.
	 ...
	 * @since 1.0
	 */
	public void insertHostData(String type, String datacenterName, int hostId, double time, double amount,
			int simulationId) {
	 ...
	 * @param datacenterName the name of the datacenter that owns the host.
	 * @param hostId         the id of the host.
	 * @return a map with values of time as keys and values of used resources as
	 *         values.
	 * @see ReportData
	 * @since 1.0
	 */
	 ...
	 * @param time         the instant of time which the data being inserted refers
	 *                     to.
	 * @param amount       the amount of resources being used by the virtual machine
	 *                     at <code>time</code>.
	 * @param simulationId the identification number of the simulation.
	 *
	 * @see ReportData
	 * @since 1.0
	 */
	public void insertVmData(String type, String customerName, int vmId, double time, double amount, int simulationId) {
	 ...
	 * @param time           the instant of time which the data being inserted
	 *                       refers to.
	 * @param amount         the overall amount of resources being used by the
	 *                       datacenter at <code>time</code>.
	 * @param simulationId   the identification number of the simulation.
	 * 
	 * @see ReportData
	 * @since 1.0
	 */
	public void insertDatacenterOverallData(String type, String datacenterName, double time, double amount,
			int simulationId) {
	...
	public TreeMap<Double, Double> getDatacenterOverallData(String type, String datacenterName) {
		...
		try {
			dataList = (List<ReportData>) session.createCriteria(ReportData.class)
					.add(Restrictions.conjunction().add(Restrictions.eq("type", type))
							.add(Restrictions.eq("datacenterName", datacenterName)).add(Restrictions.isNull("hostId")))
					.list();
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, null, ex);
		} finally {
	 ...
	 *                     to.
	 * @param amount       the overall amount of resources being used by the
	 *                     customer at <code>time</code>.
	 * @param simulationId the identification number of the simulation.
	 * 
	 * @see ReportData
	 * @since 1.0
	 */
	public void insertCustomerOverallData(String type, String customerName, double time, double amount,
			int simulationId) {
