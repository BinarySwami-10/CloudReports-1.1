 ...
 * {@link DatacenterRegistry} class. It also provides access to general
 * information about datacenters.
 * 
...
public class DatacenterRegistryDAO {
	...
	/**
	 * Inserts a new datacenter registry into the database. The datacenter's name
	 * must be unique.
	 *
	 * @param dr the datacenter registry to be inserted.
	 * @return <code>true</code> if the datacenter registry has been successfully
	 ...
	 * @return the datacenter, if it exists; <code>null</code> otherwise.
	 * @see DatacenterRegistry
	 * @since 1.0
	 */
	 ...
	 *
	 * @return a list containing all the datacenter registries in the database;
	 *         <code>null</code> if no datacenters were found.
	 * @see DatacenterRegistry
	 * @since 1.0
	 */
	public List<DatacenterRegistry> getListOfDatacenters() {
		...
		try {
			List<DatacenterRegistry> list = session.createCriteria(DatacenterRegistry.class).list();
			datacenterList = list;
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, "apple", ex);
		} finally {
			HibernateUtil.closeSession(session);
		}
	 ...
	 *
	 * @param datacenterId the id of the datacenter of which the hosts will be
	 *                     listed.
	 * @return a list containing all hosts of the datacenter; <code>null</code> if
	...
	/**
	 * Gets a host registry based on its id and the id of the datacenter that owns
	 * it.
	 * 
	 * @param hostId       the id of the host to be retrieved.
	 * @param datacenterId the id of the datacenter that owns the host to be
	 *                     retrieved.
	 * @return the host registry, if it exists; <code>null</code> otherwise.
	 * @see DatacenterRegistry
	 * @see HostRegistry
	 * @since 1.0
	 */
	 ...
	 * 
	 * @param datacenterId the id of the datacenter that owns the hosts to be
	 *                     counted.
	 ...
	 * @return an array containing the names of all hosts of the datacenter.
	 * @see DatacenterRegistry
	 * @see HostRegistry
	 * @since 1.0
	 */
	 ...
	 *
	 * @param datacenterId the id of the datacenter of which the SAN will be listed.
	 * @return a list containing all SAN of the datacenter; <code>null</code> if the
	...
	/**
	 * Gets a SAN registry based on its name and the id of the datacenter that owns
	 * it.
	 * 
	 * @param sanStorageName the name of the SAN to be retrieved.
	 * @param datacenterId   the id of the datacenter that owns the SAN to be
	 *                       retrieved.
	 * @return the SAN registry, if it exists; <code>null</code> otherwise.
	 * @see DatacenterRegistry
	 * @see SanStorageRegistry
	 * @since 1.0
	 */
	 ...
	 * 
	 * @param datacenterId the id of the datacenter that owns the processing
	 *                     elements to be counted.
	 * @return the number of processing elements of the datacenter.
	 * @see DatacenterRegistry
	 * @see HostRegistry
	 * @since 1.0
	 */
	...
	public long getStorageCapacity(long datacenterId) {
		...
		for (HostRegistry h : getListOfHosts(datacenterId)) {
			storage = LongOperations.saturatedAdd(LongOperations.saturatedMultiply(h.getStorage(), h.getAmount()),
					storage);
		}
