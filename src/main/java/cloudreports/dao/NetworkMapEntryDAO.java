...
public class NetworkMapEntryDAO {
	 ...
	 * @return <code>true</code> if the initial entries have been successfully
	...
	public boolean insertNewEntityEntries(String entityName) {
		...
		try {
			NetworkMapEntry ne = (NetworkMapEntry) session.createCriteria(NetworkMapEntry.class)
					.add(Restrictions.eq("source", entityName)).uniqueResult();
		...
		for (DatacenterRegistry dr : datacenterList) {
			/*
			 * Include all other datacenters as a destinations to this new entity
			 */
			NetworkMapEntry newEntry = new NetworkMapEntry(entityName, dr.getName(), 1.0, 1.0);
			insertNetworkMapEntry(newEntry);

			/*
			 * Include this new entity as a destination to all other datacenters.
			 */
		...
		for (CustomerRegistry cr : customerList) {
			/*
			 * Include all other customers as a destination to this new entity
			 */
			NetworkMapEntry newEntry = new NetworkMapEntry(entityName, cr.getName(), 1.0, 1.0);
			insertNetworkMapEntry(newEntry);

			/*
			 * Include this new entity as a destination to all other customers.
			 */
	 ...
	 * @return the network entry, if it exists; <code>null</code> otherwise.
	 * @see NetworkMapEntry
	 * @since 1.0
	 */
	public NetworkMapEntry getNetworkMapEntry(String source, String destination) {
		...
		try {
			entry = (NetworkMapEntry) session.createCriteria(NetworkMapEntry.class).add(
					Restrictions.and(Restrictions.eq("source", source), Restrictions.eq("destination", destination)))
					.uniqueResult();
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, null, ex);
		} finally {
	 ...
	 *
	 * @param entityName the name of the entity of which all network entries will be
	 *                   removed.
	 * @see NetworkMapEntry
	 * @since 1.0
	 */
	 ...
	 *
	 * @return a list containing all existing network entries in the database;
	...
	/**
	 * Gets a list of network entries of a given entity. The resulting list contains
	 * entries on which the given entity is either the source or the destination.
	 * 
	 * @param entityName the name of the entity of which all related network entries
	 *                   will be retrieved.
	 * @return a list containing all existing network entries in the database;
	 *         <code>null</code> if no entries were found.
	 * @see NetworkMapEntry
	...
	public List<NetworkMapEntry> getListOfEntries(String entityName) {
		...
		try {
			networkList = (List<NetworkMapEntry>) session.createCriteria(NetworkMapEntry.class).add(
					Restrictions.or(Restrictions.eq("source", entityName), Restrictions.eq("destination", entityName)))
					.list();
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, null, ex);
		} finally {
	 ...
	 *
	 * @param entityName the name of the entity of which all destinations will be
	 *                   retrieved.
	 * @return a list containing all network entries on which the given entity is
	 *         the source; <code>null</code> if no entries were found.
	 * @see NetworkMapEntry
	 * @since 1.0
	 */
	public List<NetworkMapEntry> getListOfDestinations(String entityName) {
		...
		try {
			networkList = (List<NetworkMapEntry>) session.createCriteria(NetworkMapEntry.class)
					.add(Restrictions.eq("source", entityName)).list();
		} catch (HibernateException ex) {
			Logger.getLogger(Database.class.getName()).log(Level.SEVERE, null, ex);
		} finally {
			HibernateUtil.closeSession(session);
