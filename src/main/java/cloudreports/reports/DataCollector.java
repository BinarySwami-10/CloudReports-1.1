...
public class DataCollector {
	...
	/**
	 * The number of samples to be considered when dealing with monitored resources.
	 */
	...
	/**
	 * Collects data from virtual machines and hosts and adds them to a transient
	 * list. If the transient list's size gets greater than 1000, the data is
	...
	private void collectVmsData() {
		...
		for (String brokerName : brokerNames) {
			...
			double overallRam = 0, overallCpu = 0, overallBandwidth = 0;
			...
			for (Vm vm : vmsList) {
				...
				dataList.add(new ReportData(RAM, null, brokerName, null, vmId, currentTime, ramUtilization,
						currentSimulation));
				overallRam += ramUtilization;

				double cpuUtilization = (vm.getCurrentRequestedTotalMips() / vm.getMips()) * 100;
				dataList.add(new ReportData(CPU, null, brokerName, null, vmId, currentTime, cpuUtilization,
						currentSimulation));
				overallCpu += cpuUtilization;

				double bwUtilization = (vm.getCurrentAllocatedBw() / vm.getBw()) * 100;
				dataList.add(new ReportData(BANDWIDTH, null, brokerName, null, vmId, currentTime, bwUtilization,
						currentSimulation));
			...
			dataList.add(new ReportData(BANDWIDTH, null, brokerName, currentTime, overallBandwidth / numOfVms,
					currentSimulation));
		}
	...
	private void collectHostsData() {
		...
		for (String datacenterName : datacenterNames) {
			...
			double overallRam = 0, overallCpu = 0, overallBandwidth = 0, overallPower = 0;
			int currentSimulation = SettingBusiness.getCurrentSimulation();
			...
			for (PowerHost host : hostsList) {
				...
				dataList.add(new ReportData(RAM, datacenterName, null, hostId, null, currentTime, ramUtilization,
						currentSimulation));
				overallRam += ramUtilization;

				double cpuUtilization = (host.getUtilizationOfCpuMips() / host.getTotalMips()) * 100;
				dataList.add(new ReportData(CPU, datacenterName, null, hostId, null, currentTime, cpuUtilization,
						currentSimulation));
				overallCpu += cpuUtilization;

				double bwUtilization = (host.getUtilizationOfBw() / host.getBw()) * 100;
				dataList.add(new ReportData(BANDWIDTH, datacenterName, null, hostId, null, currentTime, bwUtilization,
						currentSimulation));
				overallBandwidth += bwUtilization;

				double powerUtilization = (host.getPower() / host.getMaxPower()) * 100;
				dataList.add(new ReportData(POWER, datacenterName, null, hostId, null, currentTime, powerUtilization,
						currentSimulation));
			...
			dataList.add(
					new ReportData(RAM, datacenterName, null, currentTime, overallRam / numOfHosts, currentSimulation));
			dataList.add(
					new ReportData(CPU, datacenterName, null, currentTime, overallCpu / numOfHosts, currentSimulation));
			dataList.add(new ReportData(BANDWIDTH, datacenterName, null, currentTime, overallBandwidth / numOfHosts,
					currentSimulation));
			dataList.add(new ReportData(POWER, datacenterName, null, currentTime, overallPower / numOfHosts,
					currentSimulation));
		}
	}
	 ...
	 * 
	 * @param time the simulation time at which the cleared data must be inserted.
	 * @since 1.0
	 */
	...
	public void collectMonitoredUsedResources() {
		...
		for (String datacenterName : datacenterNames) {
			...
			for (PowerHost host : hostsList) {
				...
				double ramUtilization = (host.getUtilizationOfRam() / host.getRam()) * 100;
				insertMonitoredUsedResources(
						new ReportData(RAM, datacenterName, null, hostId, null, currentTime, ramUtilization, 0));

				double cpuUtilization = (host.getUtilizationOfCpuMips() / host.getTotalMips()) * 100;
				insertMonitoredUsedResources(
						new ReportData(CPU, datacenterName, null, hostId, null, currentTime, cpuUtilization, 0));
			}
		}
	}
	...
	private void insertMonitoredUsedResources(ReportData data) {
		...
		for (ReportData sample : this.monitoredUsedResources) {
			if (sample.getDatacenterName().equals(data.getDatacenterName())
					&& sample.getHostId().equals(data.getHostId()) && sample.getType().equals(data.getType())) {
	...
	public List<Double> getMonitoredUsedResources(String datacenterName, int hostId, String type) {
		...
		for (ReportData sample : this.monitoredUsedResources) {
			if (sample.getDatacenterName().equals(datacenterName) && sample.getHostId().equals(hostId)
