...
/**
 * Provides methods to generate a simulation report. The reports are generated
 * as a folder containing several HTML files and a raw data file.
 * 
 * @author Thiago T. Sá
 * @since 1.0
 */
public class Report {
	 ...
	 * 
	 * @throws IOException        if any of the directories or files could not be
	 *                            created.
	...
	private static void createDirectoryTree() throws IOException, URISyntaxException {
		if (htmlReportsEnabled)
			createHtmlDirectoryTree();
		if (rawDataReportsEnabled)
			createRawDataDirectoryTree();
	}

	/**
	 * Creates the raw data report's directory tree.
	 * 
	 * @throws IOException if any of the directories or files could not be created.
	 * @since 1.1
	 */
	...
	/**
	 * Creates the overall report. This part of the report contains general
	 * information about the simulation and includes overall data about datacenters
	 * and customers.
	 * 
	 * @param datacentersList the list of simulated datacenters.
	 * @param brokersList     the list of simulated brokers.
	...
	private static void createOverallReport(List<PowerDatacenter> datacentersList, List<DatacenterBroker> brokersList,
		...
		overallInformation.append("Simulation has finished in ").append(elapsedTime.toString()).append(".");
		...
		for (PowerDatacenter datacenter : datacentersList) {
			...
			overallInformation.append("<br/><br/><strong>").append(datacenter.getName())
					.append(":</strong><br/>Allocation policy: ").append(dcRegistry.getAllocationPolicyAlias())
					.append("<br/>Number of hosts: ").append(drDAO.getNumOfHosts(dcRegistry.getId()))
					.append("<br/>Number of migrations: ").append(datacenter.getMigrationCount());
		}
		...
		for (DatacenterBroker broker : brokersList) {
			...
			overallInformation.append("<br/><br/><strong>").append(broker.getName())
					.append(":</strong><br/>Broker policy: ")
					.append(custRegistry.getUtilizationProfile().getBrokerPolicyAlias())
					.append("<br/>Number of virtual machines: ").append(crDAO.getNumOfVms(custRegistry.getId()));
		}
	...
	private static void createDatacentersReports(List<PowerDatacenter> datacentersList,
		...
		for (PowerDatacenter datacenter : datacentersList) {
			DatacenterReport datacenterReport = new DatacenterReport(datacenter, brokersList, htmlReportsEnabled,
					rawDataReportsEnabled);
		...
		if (htmlReportsEnabled) {
			...
			for (DatacenterReport report : datacenterReports) {
				...
				if (datacenterReports.indexOf(report) == 0)
					datacentersHtmlList.append(report.getHtml().replace(
							"id=\"datacenter_" + report.getName() + "\" style=\"display: none;\"",
							"id=\"datacenter_" + report.getName() + "\""));
				else
	 ...
	 * @since 1.0
	 */
	private static void createCustomersReports(List<DatacenterBroker> brokersList)
		...
		if (htmlReportsEnabled) {
			...
			for (CustomerReport report : customerReports) {
				...
				if (customerReports.indexOf(report) == 0)
					customersHtmlList.append(report.getHtml().replace(
							"id=\"customer_" + report.getName() + "\" style=\"display: none;\"",
							"id=\"customer_" + report.getName() + "\""));
				else
	 ...
	 * @since 1.0
	 */
	private static void createLogReport(List<PowerDatacenter> datacentersList, List<DatacenterBroker> brokersList)
			throws IOException, URISyntaxException {
