...
/**
 * Provides methods to generate a log with general information about the
 * simulation process.
 * 
 * @author Thiago T. Sá
 * @since 1.0
...
class LogReport {
	 ...
	 * @since 1.0
	 */
	LogReport(List<PowerDatacenter> datacentersList, List<DatacenterBroker> brokersList)
	 ...
	 * 
	 * @param list       the list of cloudlets whose information will be included in
	 *                   the table.
	...
	private void printCloudletList(List<Cloudlet> list, String brokerName) {
		...
		Log.print("<tr>" + startColumn + "Cloudlet ID" + endColumn + startColumn + "STATUS" + endColumn + startColumn
				+ "Resource ID" + endColumn + startColumn + "VM ID" + endColumn + startColumn + "Time" + endColumn
				+ startColumn + "Start Time" + endColumn + startColumn + "Finish Time" + endColumn + "</tr>");

		DecimalFormat dft = new DecimalFormat("###.##");
		for (int i = 0; i < size; i++) {
			cloudlet = list.get(i);

			Log.print("<tr>" + startColumn + cloudlet.getCloudletId() + endColumn + startColumn
					+ cloudlet.getCloudletStatusString() + endColumn + startColumn + cloudlet.getResourceId()
					+ endColumn + startColumn + cloudlet.getVmId() + endColumn + startColumn
					+ dft.format(cloudlet.getActualCPUTime()) + endColumn + startColumn
					+ dft.format(cloudlet.getExecStartTime()) + endColumn + startColumn
					+ dft.format(cloudlet.getFinishTime()) + endColumn + "</tr>");
		}
		Log.print("</table>");
	}
