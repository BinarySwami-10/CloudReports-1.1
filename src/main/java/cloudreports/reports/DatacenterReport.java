...
/**
 * Provides methods to generate simulation reports with information about usage
 * of resources by datacenters.
 * 
 * @author Thiago T. Sá
 * @since 1.0
 */
class DatacenterReport {
	...
	/**
	 * The raw data. This is useful for importing simulation data to other
	 * softwares, such as Octave or MATLAB.
	 */
	...
	public DatacenterReport(PowerDatacenter datacenter, List<DatacenterBroker> brokersList, boolean htmlReportsEnabled,
		...
		for (DatacenterBroker broker : brokersList) {
			...
			// Set the number of virtual machines from this customer deployed on this
			// datacenter
	...
	/**
	 * Generates the HTML version of the datacenter's report. It reads the template
	 * files and replaces the existing tags with real simulation data. The resulting
	 * string is assigned to the {@link #html} field.
	 * 
	 * @throws IOException        If the template files could not be loaded
	 *                            correctly.
	 * @throws URISyntaxException If the path to the template files could not be
	 *                            parsed successfully.
	 * @since 1.0
	 */
	private void generateHtml() throws IOException, URISyntaxException {
		...
		for (Integer hostId : hostIds) {
			hostResUtilizationOptions.append("<option value=\"resource_utilization_").append(this.name).append("_")
					.append("Host").append(hostId).append("\">Host").append(hostId).append("</option>\n");

			hostPowerConsumptionOptions.append("<option value=\"power_consumption_").append(this.name).append("_")
					.append("Host").append(hostId).append("\">Host").append(hostId).append("</option>\n");

			// Create host resource utilization html
			String resHtml = FileIO
					.readStringFromResource("cloudreports/gui/reports/resources/host_resource_utilization");
			...
			String powerHtml = FileIO
					.readStringFromResource("cloudreports/gui/reports/resources/host_power_consumption");
			powerHtml = powerHtml.replace("<!--INSERT_HOST_NAME-->", "Host" + hostId);
			powerHtml = powerHtml.replace("<!--INSERT_POWER_CONSUMPTION_DATA-->",
					getDataString(hostsUsedPower.get(hostId)));
	...
	private String getDataString(TreeMap<Double, Double> dataMap) {
		...
		while (iterator.hasNext()) {
			...
			dataStringBuilder.append("[").append(time).append(",").append(dataMap.get(time)).append("],");
		}
	...
	/**
	 * Converts a label-value map of generic data to a string format that can be
	 * parsed by the Flot library.
	 * 
	 * @param dataMap a map of generic report data.
	 * @return a formated string containing the data of the map.
	...
	private String getDataAndLabelString(TreeMap<String, Double> dataMap) {
		...
		while (iterator.hasNext()) {
			...
			dataStringBuilder.append("{ data: [[").append(i) // The x label is not shown here
					.append(",").append(dataMap.get(customer)).append("]], label: \"").append(customer).append("\"},");
		}
		dataStringBuilder.deleteCharAt(dataStringBuilder.lastIndexOf(","));
		dataStringBuilder.append("]");
	...
	/**
	 * Generates the report's raw data and assigns it to the {@link #rawData} field.
	 * 
	 * @since 1.0
	 */
	private void generateRawData() throws IOException, URISyntaxException {
		...
		rawDataStringBuilder
				.append(getRawDataString(overallUsedRam, "Sim" + simulationId + "_" + this.name + "_overall_ram"));
		rawDataStringBuilder
				.append(getRawDataString(overallUsedCpu, "Sim" + simulationId + "_" + this.name + "_overall_cpu"));
		rawDataStringBuilder
				.append(getRawDataString(overallUsedBandwidth, "Sim" + simulationId + "_" + this.name + "_overall_bw"));
		rawDataStringBuilder
				.append(getRawDataString(overallUsedPower, "Sim" + simulationId + "_" + this.name + "_overall_power"));

		List<Integer> hostIds = Arrays.asList(hostsUsedRam.keySet().toArray(new Integer[0]));
		for (Integer hostId : hostIds) {
			rawDataStringBuilder.append(getRawDataString(hostsUsedRam.get(hostId),
					"Sim" + simulationId + "_" + this.name + "_host" + hostId + "_ram"));
			rawDataStringBuilder.append(getRawDataString(hostsUsedCpu.get(hostId),
					"Sim" + simulationId + "_" + this.name + "_host" + hostId + "_cpu"));
			rawDataStringBuilder.append(getRawDataString(hostsUsedBandwidth.get(hostId),
					"Sim" + simulationId + "_" + this.name + "_host" + hostId + "_bw"));
			rawDataStringBuilder.append(getRawDataString(hostsUsedPower.get(hostId),
					"Sim" + simulationId + "_" + this.name + "_host" + hostId + "_power"));
		}
	...
	/**
	 * Converts a time-value map to a string that can be easily parsed by a third
	 * party software.
	 * 
	 * @since 1.0
	 */
	...
	private String getMigrationsString() {
		...
		for (Migration migration : migrationList) {
			migrationStringBuilder.append("<br/><hr>").append("<strong>Migration ")
					.append(migrationList.indexOf(migration)).append(":</strong><br/>").append("Description: ")
					.append(migration.getDescription()).append("<br/><strong>").append(migration.getVmLabel())
					.append("</strong> from <strong>").append(migration.getSourceHostLabel())
					.append("</strong> to <strong>").append(migration.getTargetHostLabel())
					.append("</strong> at <strong>").append(migration.getTime() / 60).append("</strong> minutes.<br/>")
					.append("Source host was consuming ").append(migration.getSourceHostCpuUtilization() * 100)
					.append("% of CPU, ").append(migration.getSourceHostRamUtilization() * 100).append("% of RAM and ")
					.append(migration.getSourceHostPowerConsumption() * 100).append("% of power.<br/>")
					.append("Target host was consuming ").append(migration.getTargetHostCpuUtilization() * 100)
					.append("% of CPU, ").append(migration.getTargetHostRamUtilization() * 100).append("% of RAM and ")
					.append(migration.getTargetHostPowerConsumption() * 100).append("% of power.<br/>");
		}
