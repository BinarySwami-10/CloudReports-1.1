...
class CustomerReport {
	 ...
	 * @since 1.0
	 */
	public CustomerReport(DatacenterBroker broker, boolean htmlReportsEnabled, boolean rawDataReportsEnabled)
	...
	/**
	 * Generates the HTML version of the customer's report. It reads the template
	...
	private void generateHtml() throws IOException, URISyntaxException {
		...
		for (Integer vmId : vmIds) {
			vmResUtilizationOptions.append("<option value=\"resource_utilization_").append(this.name).append("_")
					.append("VM").append(vmId).append("\">VM").append(vmId).append("</option>\n");

			// Create virtual machine resource utilization html
			String resHtml = FileIO
					.readStringFromResource("cloudreports/gui/reports/resources/vm_resource_utilization");
	...
	private String[] getCloudletsTimesDataString(TreeMap<Integer, List<Double>> dataMap) {
		...
		if (startStringBuilder.lastIndexOf(",") >= 0)
			startStringBuilder.deleteCharAt(startStringBuilder.lastIndexOf(","));
		startStringBuilder.append("]");
		if (finishStringBuilder.lastIndexOf(",") >= 0)
			finishStringBuilder.deleteCharAt(finishStringBuilder.lastIndexOf(","));
		finishStringBuilder.append("]");
		...
		String averageStartData = "[[" + cloudletIdsList.get(0) + "," + averageStart + "],["
				+ cloudletIdsList.get(numberOfCloudlets - 1) + "," + averageStart + "]]";

		double averageFinish = finishSum / numberOfCloudlets;
		String averageFinishData = "[[" + cloudletIdsList.get(0) + "," + averageFinish + "],["
				+ cloudletIdsList.get(numberOfCloudlets - 1) + "," + averageFinish + "]]";

		return new String[] { startStringBuilder.toString(), finishStringBuilder.toString(), averageStartData,
				averageFinishData };
	}
	...
	private void generateRawData() {
		...
		List<Integer> vmIds = Arrays.asList(vmsUsedRam.keySet().toArray(new Integer[0]));
		for (Integer vmId : vmIds) {
			rawDataStringBuilder.append(getRawDataString(vmsUsedRam.get(vmId),
					"Sim" + simulationId + "_" + this.name + "_vm" + vmId + "_ram"));
			rawDataStringBuilder.append(getRawDataString(vmsUsedCpu.get(vmId),
					"Sim" + simulationId + "_" + this.name + "_vm" + vmId + "_cpu"));
			rawDataStringBuilder.append(getRawDataString(vmsUsedBandwidth.get(vmId),
					"Sim" + simulationId + "_" + this.name + "_vm" + vmId + "_bw"));
		}
