...
/**
 * Defines native types of bandwidth provisioners and implements an extension
 * type to support user-implemented new types.
 * 
 * @author Thiago T. Sá
 * @since 1.0
 */
public enum BwProvisioner {
	...
	/**
	 * The simple bandwidth provisioner. Its
	 * {@link #getProvisioner(int, java.lang.String)} method returns an instance of
	 * CloudSim's BwProvisionerSimple class.
	 */
	SIMPLE {
		@Override
	...
	EXTENSION {
		...
		public org.cloudbus.cloudsim.provisioners.BwProvisioner getProvisioner(long num, String provisionerAlias) {
			try {
				...
				return (org.cloudbus.cloudsim.provisioners.BwProvisioner) ExtensionsLoader.getExtension("BwProvisioner",
						provisionerAlias, types, arguments);
			} catch (Exception e) {
				return null;
			}
		}
	 ...
	 *
	 * @param num              the overall amount of bandwidth available in the
	 *                         host.
	...
	public static BwProvisioner getInstance(String provisionerAlias) {
		if (provisionerAlias.equals("Simple"))
			return BwProvisioner.SIMPLE;
		else
			return BwProvisioner.EXTENSION;
	}

	/**
	 * Gets all active bandwidth provisioners aliases.
	 *
	 * @return an array of strings containing all active bandwidth provisioners
	 *         aliases.
	 * @since 1.0
	 */
