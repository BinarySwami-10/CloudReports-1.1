...
public class Simulation implements Runnable {
	...
	/**
	 * Runs all simulations of a specific environment. It creates all CloudSim
	 * entities, runs the simulation and generates the report. This cycle is
	 * repeated the number of times indicated by the NumberOfSimulations setting.
	 * 
	 * @since 1.0
	 */
	private void runAllSimulations() {
		...
		for (int simulationId = 1; simulationId <= numberOfSimulations; simulationId++) {
			...
			MainView.getSimulationView().setBarLabel(
					"Simulation " + simulationId + " of " + HibernateUtil.getActiveDatabase() + " is in progress...");
		...
		try {
			...
			if (hasTerminated()) {
				...
				if (isMailNotificationEnabled) {
					Mail.sendMail(
							"Simulation " + simulationId + " of " + HibernateUtil.getActiveDatabase() + " completed.",
							"Simulation " + simulationId + " of " + HibernateUtil.getActiveDatabase()
									+ " has completed in " + elapsedTime.toString() + ".");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
			...
			if (isMailNotificationEnabled) {
				...
				Mail.sendMail("Simulation " + simulationId + " of " + HibernateUtil.getActiveDatabase() + " failed.",
						"Simulation " + simulationId + " of " + HibernateUtil.getActiveDatabase() + " has failed after "
