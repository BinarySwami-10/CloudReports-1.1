 ...
 * @since 1.0
 */
public class VmAllocationPolicySingleThreshold extends VmAllocationPolicySimple
		implements VmAllocationPolicyExtensible {

	private double upperUtilizationThreshold;

	/**
	 * Initializes a new instance of this class with the given list of hosts and
	...
	/**
	 * Optimizes the allocation of virtual machines. It distributes virtual machines
	 * if there is any overused host and then consolidates the allocation.
	 *
	...
	public void consolidateVms(List<Migration> migrationList) {
		...
		for (PowerHost sourceHost : sourceHosts) {
			...
			getConsolidatingMigrationList(newMigrationList, sourceHost, targetHosts, hostsToBeTurnedOffIds,
	 ...
	 *
	 * @return <code>true</code> if the host could be allocated; <code>false</code>
	 *         otherwise.
	 */
	@Override
	...
	public PowerHost findHostForVm(Vm vm) {
		...
		for (PowerHost host : this.<PowerHost>getHostList()) {
			if (host.isSuitableForVm(vm)) {
				...
				if ((!vm.isBeingInstantiated() && maxUtilization > getUpperUtilizationThreshold())
						|| (vm.isBeingInstantiated() && maxUtilization > 1.0)) {
					continue;
				}
				try {
	...
	/**
	 * Calculates a list of migrations needed to distribute virtual machines among
	 * hosts.
	 *
	 * @param migrationList             a list o migrations to be calculated.
	 * @param sourceHost                the host from which virtual machines will be
	 *                                  taken to be reallocated in other hosts.
	 * @param targetHosts               a list of target hosts.
	 * @param upperUtilizationThreshold the upper utilization threshold.
	 * @since 1.0
	 */
	protected void getDistributingMigrationList(List<Migration> migrationList, PowerHost sourceHost,
			List<PowerHost> targetHosts, double upperUtilizationThreshold) {
		...
		VMS_LOOP: for (Vm vm : sourceHost.getVmList()) {
			if (vm.isInMigration()) {
				continue;
			}

			HOSTS_LOOP: for (PowerHost targetHost : targetHosts) {
				...
				if ((hostResources.get(0) + vmTotalMips) / targetHost.getTotalMips() < upperUtilizationThreshold
					...
					// There's enough resources and no threshold violation, so add the migration to
					// the list.
	 ...
	 * @since 1.0
	 */
	protected void getConsolidatingMigrationList(List<Migration> migrationList, PowerHost sourceHost,
			List<PowerHost> targetHosts, List<Integer> hostsToBeTurnedOffIds, double upperUtilizationThreshold) {
		// Map of resources
	...
	public static double getHostCpuUtilizationRate(PowerHost host) {
		...
		List<Double> previousValuesCpu = Simulation.getDataCollector()
				.getMonitoredUsedResources(host.getDatacenter().getName(), host.getId(), "CPU");
	...
	public static double getHostRamUtilizationRate(PowerHost host) {
		...
		List<Double> previousValuesRam = Simulation.getDataCollector()
				.getMonitoredUsedResources(host.getDatacenter().getName(), host.getId(), "RAM");
