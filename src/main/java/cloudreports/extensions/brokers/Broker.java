...
public abstract class Broker extends DatacenterBroker {
	...
	protected void processVmCreate(SimEvent ev) {
		...
		if (result == CloudSimTags.TRUE) {
			...
			Log.printLine(CloudSim.clock() + ": " + getName() + ": VM #" + vmId + " has been created in "
					+ getDatacenterCharacteristicsList().get(datacenterId).getResourceName() + ", Host #"
					+ VmList.getById(getVmsCreatedList(), vmId).getHost().getId());
		} else {
			Log.printLine(CloudSim.clock() + ": " + getName() + ": Creation of VM #" + vmId + " failed in "
					+ getDatacenterCharacteristicsList().get(datacenterId).getResourceName());
		}

		incrementVmsAcks();

		if (getVmsCreatedList().size() == getVmList().size() - getVmsDestroyed()) { // all the requested VMs have been
																					// created
			submitCloudlets();
		} else {
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		Cloudlet newCloudlet = new Cloudlet(this.cloudletId,
				...
				cloudlet.getNumberOfPes(), cloudlet.getCloudletLength(), cloudlet.getCloudletOutputSize(),
	...
	/**
	 * An abstract method whose implementations must return an id of a datacenter.
	 *
