...
public class PowerDatacenter extends org.cloudbus.cloudsim.power.PowerDatacenter {
	...
	public PowerDatacenter(String name, DatacenterCharacteristics characteristics,
			...
			double monitoringInterval) throws Exception {
		...
		if (currentTime > getLastProcessTime()) {
			...
			if ((currentTime - this.lastMonitoringTime) >= this.getMonitoringInterval()) {
				...
				if (!isDisableMigrations()) {
					...
					List<Migration> migrationList = ((VmAllocationPolicyExtensible) getVmAllocationPolicy())
							.getListOfMigrationsToBeExecuted(getVmList());
					...
					for (Migration migration : migrationList) {
						...
						if (oldHost == null) {
							Log.formatLine("%.2f: Migration of VM #%d to Host #%d has started", CloudSim.clock(),
									vm.getId(), targetHost.getId());
						} else {
							Log.formatLine("%.2f: Migration of VM #%d from Host #%d to Host #%d has started",
						...
						send(getId(), vm.getRam() / ((double) vm.getBw() / 8000) + 10, CloudSimTags.VM_MIGRATE,
								migration.getMigrationMap());
					}

					if (!migrationList.isEmpty())
						Simulation.getDataCollector().flushMigrations(migrationList);
				}
